...
public class ProductReturnQueue {
	...
	public int remove(ItemStack item, int count) {
		...
		while (remaining > 0 && !queue.isEmpty()) {
			...
			if (promise.count() <= remaining) {
				...
				leftovers.add(new AddressPromise(promise.item(), promise.count() - remaining, promise.address(),
	...
	/** Smallest outstanding promise for the item, or {@code null}. */
	public AddressPromise peek(ItemStack item) {
	...
	/** Advances the age of every queued promise by one tick. */
	public void tick() {
		tick(1);
	}
